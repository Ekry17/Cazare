...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // PAYMENT INTENT ENDPOINT - pentru plăți Stripe
    if (url === '/api/create-payment-intent' && method === 'POST') {
      const { amount, currency, reservationData } = req.body;
      
      try {
        // În mod normal aici ar fi integrarea cu Stripe
        // Pentru demo, returnez un răspuns mock
        const paymentIntent = {
    ...
    // CONFIRM PAYMENT ENDPOINT - confirmă plata
    if (url === '/api/confirm-payment' && method === 'POST') {
      const { paymentIntentId, reservationData } = req.body;
      
      try {
        // În mod normal aici ar fi confirmarea cu Stripe
        // Pentru demo, simulez succesul
        console.log('✅ PLATĂ CONFIRMATĂ:', {
          paymentIntentId: paymentIntentId,
          reservation: reservationData
        });

        // Salvez rezervarea cu status "PLĂTITĂ"
        const reservation = {
          id: Date.now(),
          ...reservationData,
          status: 'PAID',
        ...
        // Trimit notificare către proprietar
